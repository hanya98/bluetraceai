...
async def run_full_analysis(
    ...
    top_vessel_id = top_vessels[0].get("vessel_id", "N/A") if top_vessels else "None"
    summary = (
        f"Oil spill detected ({detection_res['area_km2']} km²) with {classification_res['oil_probability']*100:.1f}% "
        f"oil confidence. Candidate vessel {top_vessel_id} ranked highest priority for verification."
    )
